...
.DESCRIPTION
    Skript zum erstellen eine Verzeichnisses welches eine angegebene Anzahl von Dateien und Ordnern beeinhaltet f�r TestZwecke
.PARAMETER DirCount
...
.PARAMETER force
    Wird der Force Parameter angegeben wird ein vorhandenes Verzeichniss komplett gel�scht um es anschlie�end neu zu erstelltn.
.EXAMPLE
...
Write-Progress -Activity "Erstelle TestFiles Ordner" -Status "Pr�fung auf vorhandene Dateien" -PercentComplete 25
if((Test-Path -Path $Path) -and $force)
{
    Write-Verbose -Message "vorhandene Ordner und Dateien werden gel�scht"
    Remove-Item -Path $Path -Recurse -Force | Out-Null
}
if((Test-Path -Path $Path) -and $force -eq $false)
{
    Write-Host -ForegroundColor Red "Ordner bereits vorhanden, l�schen sie das Verzeichnis manuell oder rufen Sie das Skript erneut mit dem Parameter -force aus zum automatischen vorherigen l�schen des Verzeichnisses inklusive aller Dateien und Unterordner"
}
else
{
Write-Progress -Activity "Erstelle TestFiles Ordner" -Status "Ordnererstellung" -PercentComplete 50
    ...
    {
        $targetcounter ++
        Write-Progress -Activity "Erstelle TestFiles Ordner" -Id 1 -Status "Dateierstellung f�r Ordner: $targetdir" -PercentComplete (100/$Ordnerpfade.Count * $targetcounter)
...
<#
.SYNOPSIS 
   Fr�gt Anmelde bezogene Events vom System ab
.DESCRIPTION
   Lange Beschreibung
.EXAMPLE
   .\Get-LogonEvents.ps1 -EventId 4624

   Fr�gt alle Anmeldeevents vom Standard Log Security ab.
...
#Ausf�hrliche Ausgabe wenn Skript mit -Verbose aufgerufen wird
Write-Verbose -Message "Es wurden vom user folgende Werte �bergeben: EventId = $EventId"
